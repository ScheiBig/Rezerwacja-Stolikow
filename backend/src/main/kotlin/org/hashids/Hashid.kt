...
interface Hashidable {
     ...
     */
    @Throws(IllegalArgumentException::class)
    fun encodeHex(hex: String): String
...
class Hashids(
     ...
     */
    @Throws(IllegalArgumentException::class)
    override fun encode(vararg numbers: Long): String = when {
     ...
     * @return Decoded numbers
     */
    override fun decode(hash: String): LongArray = when {
        hash.isEmpty() -> longArrayOf()
            ...
            @Suppress("CheckedExceptionsKotlin") when {
    ...
    private fun calculateAlphabetAndSeparators(userAlphabet: String): AlphabetAndSeparators {
        ...
        when {
            uniqueAlphabet.length < minimalAlphabetLength -> throw IllegalArgumentException(
                "alphabet must contain at least $minimalAlphabetLength unique characters"
            )
